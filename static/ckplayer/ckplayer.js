...
! (function() {
	...
	!function() {
		...
		javascriptPath = '/static/ckplayer/';
		// 使用自定义的播放路径
    // if(!javascriptPath.endsWith("static/ckplayer/")){
    //   javascriptPath += 'static/ckplayer/' ;
    // }
	} ();
	...
	ckplayer.prototype = {
		...
		*/
		embed: function(c) {
			...
			// 控制进度的属性
			if ( this.vars['configJson'] ){
        for (k in this.vars['configJson']) {
          console.log("---------配置=="+k)
          this.ckplayerConfig.config[k] = this.vars['configJson'][k];
        }
      }

      // console.log('---------自定义配置：'+ JSON.stringify(this.vars['custom']))
		...
		getVideo: function() {
			...
			if (this.html5Video) { //如果支持HTML5-VIDEO则默认使用HTML5-VIDEO播放器
				...
				if (this.isM3u8) {
					...
					this.loadJs(javascriptPath + 'hls/hls.js', loadJsHandler);
				}
